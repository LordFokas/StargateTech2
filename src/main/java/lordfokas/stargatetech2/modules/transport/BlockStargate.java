...
public class BlockStargate extends BaseBlockContainer__OLD_AND_FLAWED implements ITabletAccess {
	...
	public BlockStargate() {
		...
		setRenderer(RenderStargateBlock.instance());
	}
	...
	@Override
	public boolean onTabletAccess(EntityPlayer player, World world, int x,
			int y, int z) {
	...
	public boolean onBlockActivated(World w, int x, int y, int z,
			EntityPlayer p, int s, float hx, float hy, float hz) {
		...
		if (item instanceof IToolWrench) {
			...
			if (te instanceof ITileStargateBase && wrench.canWrench(p, x, y, z)) {
				...
				if (MinecraftForge.EVENT_BUS
						.post(new StargateEvent.StargateWrenched(stargate
								.getAddress(), stargate.getWorldObj(),
								stargate.xCoord, stargate.yCoord,
								stargate.zCoord)))
					return false;
	...
	public void setBaseOverride() {
		IIcon bottom = IconRegistry.blockIcons
				.get(TextureReference.MACHINE_BOTTOM);
		IIcon top = IconRegistry.blockIcons
				.get(TextureReference.STARGATE_BASE_TOP);
	...
	protected void registerBlock() {
		GameRegistry.registerBlock(this, ItemBlockStargate.class,
				getUnlocalizedName());
	}
	...
	public void onBlockAdded(World w, int x, int y, int z) {
		...
		if (m != META_RING)
			MinecraftForge.EVENT_BUS
					.post(new BusEvent.AddToNetwork(w, x, y, z));
	}
	...
	public void breakBlock(World w, int x, int y, int z, Block b, int m) {
		...
		if (m != META_RING)
			MinecraftForge.EVENT_BUS.post(new BusEvent.RemoveFromNetwork(w, x,
					y, z));
	}
}