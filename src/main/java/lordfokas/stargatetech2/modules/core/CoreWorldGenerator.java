...
public class CoreWorldGenerator implements IWorldGenerator {
	...
	public void generate(Random random, int cX, int cZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider){
		if(world.provider.getDimension() == 1 || world.provider.getDimension() == -1 || ConfigServer.wgNaquadah < 1) return;
		...
		WorldGenMinable wgm = new WorldGenMinable(ModuleCore.naquadahBlock.ore, 20);
